...
var currentMinutes;
var todayDate;
var selectedDate = null; // 存储选中的日期
var progressMode = 'today'; // 'today' 或 'selected'
...
document.addEventListener('DOMContentLoaded', function() {
	...
	setInterval(function() {
		// 只在当天模式下更新进度条
		if (progressMode === 'today') {
			updateProgress();
		}
	}, 60*1000);	//每分钟更新一次进度条（仅当天模式）
	// setInterval(updateTimeLine, 60*1000);
	
	// 添加按钮事件监听器
	var todayBtn = document.getElementById('view-today-progress');
	var selectedBtn = document.getElementById('view-selected-progress');
...
function updateProgress() {
	// 如果当前是选中日期模式，调用专门的函数
	if (progressMode === 'selected' && selectedDate) {
...
function updateLineAndTimer(progressBar, height, textContent, isDynamic, LorR='R') {
	...
	if (isDynamic==true) {
		smallTimer_TimeLine.className = 'hhmmss-timer'; // 根据 isDynamic 参数决定是否修改类名
	}
...
function showTodayProgress() {
	progressMode = 'today';
	updateButtonStates();
	updateProgress(); // 重新更新进度条
	console.log('切换到当天进度条模式');
}

// 显示选中日期进度条
function showSelectedProgress() {
	// 获取当前选中的日期
	...
	// 获取选中日期的信息
	var cellText = selectedCell.textContent.trim();
	if (cellText && !isNaN(cellText)) {
		selectedDate = parseInt(cellText);
		progressMode = 'selected';
		updateButtonStates();
		updateProgressForSelectedDate();
		console.log('切换到选中日期进度条模式，日期：', selectedDate);
	} else {
		alert('无法获取选中日期信息');
	}
}
...
// 为选中日期更新进度条
function updateProgressForSelectedDate() {
	...
	// 计算选中日期的星期几
	var today = new Date();
	var selectedDateObj = new Date(today.getFullYear(), today.getMonth(), selectedDate);
	var selectedDayOfWeek = selectedDateObj.getDay();
	
	console.log('选中日期：', selectedDate, '星期：', selectedDayOfWeek);
	
	// 获取选中日期的时间段
	var selectedTimeSegments = getTimeSegmentsForDate(selectedDate, selectedDayOfWeek);
	
	// 获取当前时间用于显示时间线
	getCurrentTime();
	
	// 绘制进度条（在选中日期模式下也显示当前时间线）
	var totalMinutes = 24 * 60;
	for (var i = 0; i < selectedTimeSegments.length; i++) {
		var currentSegment = selectedTimeSegments[i];
		var startTime = getTimeInMinutes(currentSegment.start);
		var endTime = getTimeInMinutes(currentSegment.end);
		...
		// 如果当前时间在这个时间段内，添加当前时间线
		if (startTime <= currentMinutes && currentMinutes <= endTime) {
			// 计算当前时间在这个时间段中的相对位置
			var timeInSegment = currentMinutes - startTime;
			var segmentDuration = endTime - startTime;
			var relativePosition = (timeInSegment / segmentDuration) * 100;
			
			// 创建当前时间线
			var currentTimeLine = document.createElement('div');
			currentTimeLine.className = 'currentTimeLine';
			currentTimeLine.style.backgroundColor = '#708090';
			currentTimeLine.style.width = '2px';
			currentTimeLine.style.height = '150%';
			currentTimeLine.style.position = 'absolute';
			currentTimeLine.style.left = relativePosition + '%';
			currentTimeLine.style.bottom = '0';
			currentTimeLine.style.zIndex = '9999';
			currentTimeLine.style.overflow = 'visible';
			
			// 创建时间显示标签
			var timeLabel = document.createElement('span');
			timeLabel.className = 'hhmmss-timer';
			timeLabel.style.position = 'absolute';
			timeLabel.style.top = '-10%';
			timeLabel.style.left = '-4vh';
			timeLabel.style.color = 'black';
			timeLabel.style.fontSize = '12px';
			timeLabel.style.fontWeight = 'bold';
			timeLabel.style.zIndex = '9999';
			
			currentTimeLine.appendChild(timeLabel);
			currentProgressBar.appendChild(currentTimeLine);
		}
		...
		if (i == selectedTimeSegments.length - 1) {
			currentProgressBar.style.borderTopRightRadius = '4vh';
			currentProgressBar.style.borderBottomRightRadius = '4vh';
		}
...
// 获取指定日期的时间段
function getTimeSegmentsForDate(date, dayOfWeek) {
	// 使用与getTodayTimeSegment()相同的时间段定义
	var timeSegments_Mon = [
		{ start: "00:00", end: "24:00", color: Color_Blank },
	];
	var timeSegments_Tues = [
	...
	if (1 <= date && date <= 15) {
		...
		if (dayOfWeek === 2 || dayOfWeek === 6) list_index = 1; // 后半月的周二周六no shard
	}
	
	return list_timeSegments[list_index];
}